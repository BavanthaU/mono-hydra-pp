...
- A computer capable of running the selected depth and semantic perception backend
...
Finally, you will need a 2D closed-set dense semantic segmentation network capable of producing category labels. In this Mono Hydra workspace, that role is handled by `mono_hydra_perception`.
...
The first step is to set up the segmentation network. In this Mono Hydra
workspace, launch `mono_hydra_perception` and remap its outputs to Hydra's
semantic input topic:
```yaml
- set_remap: {from: hydra/input/camera/semantic/image_raw, to: /camera/seg_cam/labels_argmax}
```
...
The third is that we need to set the labelspace used by the perception backend:
```yaml
  arg:
