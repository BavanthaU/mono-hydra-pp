...
2. In this Mono Hydra workspace, learned depth and semantic labels are produced by `mono_hydra_perception`; the upstream TensorRT segmentation runtime is not included.
...
In general, you can start from the `dev` profile for development if you need CUDA support with Hydra or Mono Hydra perception. You can reuse the same steps from the `minimal` profile to setup the host machine adding your software/hardware dependencies.
