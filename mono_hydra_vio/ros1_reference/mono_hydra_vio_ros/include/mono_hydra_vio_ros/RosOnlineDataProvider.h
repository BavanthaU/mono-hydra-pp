...
class RosOnlineDataProvider : public RosDataProviderInterface {
 ...
 private:
  ...
  void subscribeSemanticMask(const std::string& topic,
                             const size_t& kMaxImagesQueueSize);
  void attachLatestSemanticMask(Frame* frame) const;
  ...
  void callbackSemanticMask(const sensor_msgs::ImageConstPtr& mask_msg);
  void loadSemanticDynamicLabels();
 ...
 private:
  ...
  ros::Subscriber semantic_mask_subscriber_;
