...
RosOnlineDataProvider::RosOnlineDataProvider(const VioParams& vio_params)
  ...
  // Optional semantic feature mask. By default the topic is an 8UC1 label-ID
  // image and dynamic labels from the labelspace are converted to a binary
  // feature-detection keep mask. Set semantic_mask_is_label_image=false for an
  // already-binary mask where zero means discard and nonzero means keep.
  std::string semantic_mask_topic;
  nh_private_.param<std::string>("semantic_mask_topic",
                                 semantic_mask_topic,
                                 std::string());
  nh_private_.param<bool>("semantic_mask_is_label_image",
                          semantic_mask_is_label_image_,
                          true);
  nh_private_.param<int>("semantic_mask_inflate_px",
                         semantic_mask_inflate_px_,
                         0);
  semantic_mask_inflate_px_ = std::max(0, semantic_mask_inflate_px_);
  if (!semantic_mask_topic.empty()) {
    subscribeSemanticMask(semantic_mask_topic, kMaxImagesQueueSize);
    LOG(INFO) << "Subscribed to semantic mask topic: " << semantic_mask_topic;
    if (semantic_mask_inflate_px_ > 0) {
      LOG(INFO) << "Inflating rejected semantic-mask regions by "
                << semantic_mask_inflate_px_ << " px.";
    }
  }
...
void RosOnlineDataProvider::subscribeSemanticMask(
    const std::string& topic, const size_t& kMaxImagesQueueSize) {
  if (topic.empty()) return;
  ros::TransportHints hints = ros::TransportHints().tcpNoDelay(true);
  semantic_mask_subscriber_ = nh_private_.subscribe(
      topic,
      static_cast<uint32_t>(kMaxImagesQueueSize),
      &RosOnlineDataProvider::callbackSemanticMask,
      this,
      hints);
}
...
void RosOnlineDataProvider::callbackMonoImage(
  ...
  if (!shutdown_) {
    ...
    Frame::UniquePtr frame_ptr = std::make_unique<Frame>(
        frame_count_, timestamp, cam_info, readRosImage(img_msg));
    attachLatestSemanticMask(frame_ptr.get());
    left_frame_callback_(std::move(frame_ptr));
    frame_count_++;
  }
}
...
void RosOnlineDataProvider::callbackStereoImages(
  ...
  if (!shutdown_) {
    CHECK(left_frame_callback_)
        << "Did you forget to register the left frame callback?";
    Frame::UniquePtr left_frame_ptr = std::make_unique<Frame>(
        frame_count_, timestamp_left, left_cam_info, readRosImage(left_msg));
    attachLatestSemanticMask(left_frame_ptr.get());
    left_frame_callback_(std::move(left_frame_ptr));
...
void RosOnlineDataProvider::callbackSemanticMask(
    const sensor_msgs::ImageConstPtr& mask_msg) {
  try {
    cv_bridge::CvImageConstPtr cv_ptr =
        cv_bridge::toCvShare(mask_msg, sensor_msgs::image_encodings::TYPE_8UC1);
    cv::Mat mask = cv_ptr->image.clone();

    if (semantic_mask_is_label_image_) {
      cv::Mat keep(mask.size(), CV_8UC1, cv::Scalar(255));
      for (int lbl : dynamic_labels_) {
        cv::Mat dyn = (mask == lbl);
        keep.setTo(0, dyn);
      }
      mask = keep;
    } else {
      // Normalize common 0/1 binary masks to the conventional 0/255 range.
    ...
    LOG(WARNING) << "cv_bridge exception reading semantic mask: " << e.what();
    has_semantic_mask_ = false;
  }
}

void RosOnlineDataProvider::attachLatestSemanticMask(Frame* frame) const {
  if (!frame || !has_semantic_mask_) {
    return;
  }
...
void RosOnlineDataProvider::loadSemanticDynamicLabels() {
  // Default to NYUv2-20 dynamic labels.
  dynamic_labels_ = {19, 20};
  XmlRpc::XmlRpcValue dyn_labels;
  if (!nh_private_.getParam("semantic_labelspace/dynamic_labels", dyn_labels)) {
    VLOG(1) << "semantic_labelspace/dynamic_labels not set; using defaults.";
    return;
  }
  if (dyn_labels.getType() != XmlRpc::XmlRpcValue::TypeArray) {
    LOG(WARNING) << "semantic_labelspace/dynamic_labels is not an array; using defaults.";
    return;
  }
  std::vector<int> parsed;
  for (int i = 0; i < dyn_labels.size(); ++i) {
    if (dyn_labels[i].getType() == XmlRpc::XmlRpcValue::TypeInt) {
      parsed.push_back(static_cast<int>(dyn_labels[i]));
    } else {
      LOG(WARNING) << "semantic_labelspace/dynamic_labels contains non-int entry; skipping.";
    }
  }
  if (!parsed.empty()) {
    dynamic_labels_.swap(parsed);
    VLOG(1) << "Loaded " << dynamic_labels_.size()
            << " dynamic labels from semantic_labelspace config.";
  } else {
    LOG(WARNING) << "semantic_labelspace/dynamic_labels empty after parsing; using defaults.";
  }
}
