...
bool VioBackend::addVisualInertialStateAndOptimize(
  ...
  if (backend_params_.use_sparse_depth_factors_ && !sparse_depth_meas.empty()) {
    addSparseDepthFactors(curr_kf_id_, sparse_depth_meas);
  }
...
void VioBackend::addSparseDepthFactors(
    const FrameId& frame_id,
    const SparseDepthMeasurements& sparse_depth_meas) {
  if (sparse_depth_meas.empty()) {
    return;
  }

  const auto max_factors = backend_params_.sparse_depth_max_factors_per_kf_;
  size_t count = 0;
  size_t skipped_invalid_depth = 0;
  size_t skipped_invalid_sigma = 0;
  size_t skipped_behind_camera = 0;
  size_t skipped_reprojection = 0;
  size_t initialized_landmarks = 0;

  gtsam::Cal3_S2::shared_ptr mono_cal(
      new gtsam::Cal3_S2(stereo_cal_->calibration()));
  gtsam::SharedNoiseModel mono_noise =
      gtsam::noiseModel::Isotropic::Sigma(2, backend_params_.smartNoiseSigma_);

  const auto pose_key = gtsam::Symbol('x', frame_id);
  gtsam::Pose3 W_Pose_B_k =
      new_values_.exists(pose_key) ? new_values_.at<gtsam::Pose3>(pose_key)
                                   : state_.at<gtsam::Pose3>(pose_key);

  for (const auto& m : sparse_depth_meas) {
    if (count >= max_factors) {
      break;
    }
    if (m.depth_meas_ <= 0.0 || !std::isfinite(m.depth_meas_)) {
      ++skipped_invalid_depth;
      continue;
    }

    const double sigma = backend_params_.sparse_depth_sigma_a_ +
                         backend_params_.sparse_depth_sigma_b_ * m.depth_meas_;
    if (sigma <= 0.0 || !std::isfinite(sigma)) {
      ++skipped_invalid_sigma;
      continue;
    }

    auto base = gtsam::noiseModel::Isotropic::Sigma(1, sigma);
    auto noise = gtsam::noiseModel::Robust::Create(
        gtsam::noiseModel::mEstimator::Huber::Create(
            backend_params_.sparse_depth_huber_k_),
        base);

    const gtsam::Symbol lmk_sym('l', m.lmk_id_);
    bool initialized_here = false;
    if (!state_.exists(lmk_sym) && !new_values_.exists(lmk_sym)) {
      const double x = (m.u_ - mono_cal->px()) / mono_cal->fx() * m.depth_meas_;
      const double y = (m.v_ - mono_cal->py()) / mono_cal->fy() * m.depth_meas_;
      const double z = m.depth_meas_;
      const gtsam::Point3 p_cam(x, y, z);
      const gtsam::Point3 p_body = B_Pose_leftCamRect_.transformFrom(p_cam);
      const gtsam::Point3 p_world = W_Pose_B_k.transformFrom(p_body);
      new_values_.insert<gtsam::Point3>(lmk_sym, p_world);
      initialized_here = true;
      ++initialized_landmarks;

      // A 1D depth factor alone underconstrains a newly initialized Point3.
      new_imu_prior_and_other_factors_
          .emplace_shared<gtsam::GenericProjectionFactor<Pose3, Point3>>(
              gtsam::Point2(m.u_, m.v_),
              mono_noise,
              pose_key,
              lmk_sym,
              mono_cal,
    ...
    if (!initialized_here) {
      ...
      if (new_values_.exists(lmk_sym)) {
        p_world = new_values_.at<gtsam::Point3>(lmk_sym);
      } else {
        p_world = state_.at<gtsam::Point3>(lmk_sym);
      }
      const gtsam::Pose3 W_Pose_C = W_Pose_B_k * B_Pose_leftCamRect_;
      const gtsam::Point3 p_cam = W_Pose_C.transformTo(p_world);
      if (p_cam.z() <= 1e-6) {
        ++skipped_behind_camera;
        continue;
      }
      const double u_proj =
          mono_cal->fx() * p_cam.x() / p_cam.z() + mono_cal->px();
      const double v_proj =
          mono_cal->fy() * p_cam.y() / p_cam.z() + mono_cal->py();
      const double err_px = std::hypot(u_proj - m.u_, v_proj - m.v_);
      if (err_px > backend_params_.sparse_depth_reproj_thresh_px_) {
        ++skipped_reprojection;
        continue;
      }
    }

    auto factor = boost::make_shared<DepthFactor>(pose_key,
                                                  lmk_sym,
                                                  m.u_,
                                                  m.v_,
                                                  m.depth_meas_,
  ...
  VLOG(2) << "Sparse depth factors for keyframe " << frame_id << ": added "
          << count << " / " << sparse_depth_meas.size()
          << ", initialized_landmarks=" << initialized_landmarks
          << ", skipped_invalid_depth=" << skipped_invalid_depth
          << ", skipped_invalid_sigma=" << skipped_invalid_sigma
          << ", skipped_behind_camera=" << skipped_behind_camera
          << ", skipped_reprojection=" << skipped_reprojection;
}
  ...
  if (is_smoother_ok) {
    ...
    if (is_smoother_ok) {
      try {
        ...
        LOG(ERROR) << "Post-update backend processing failed due to cheirality: "
                   << e.what();
        is_smoother_ok = false;
      } catch (const gtsam::StereoCheiralityException& e) {
        LOG(ERROR)
            << "Post-update backend processing failed due to stereo cheirality: "
            << e.what();
        is_smoother_ok = false;
      }
    } else {
...
bool VioBackend::updateSmoother(Smoother::Result* result,
  ...
  try {
    ...
    *result =
    ...
    // Add priors on variables near the failure to fix an indeterminant linear
    // system. The nearby variable can be a pose-state key or a landmark key.
    gtsam::Values values = smoother_->calculateEstimate();
    gtsam::NonlinearFactorGraph nfg;

    const auto add_prior = [&](const gtsam::Symbol& key) {
      if (!values.exists(key)) {
        LOG(WARNING) << "Skipping recovery prior on missing key: "
                     << key.chr() << key.index();
        return;
      }
      ...
      switch (key.chr()) {
        ...
        case 'l': {
          gtsam::Point3 point = values.at<gtsam::Point3>(key);
          gtsam::Vector3 sigmas;
          sigmas.setConstant(0.1);
          gtsam::SharedNoiseModel noise =
              gtsam::noiseModel::Diagonal::Sigmas(sigmas);
          nfg.emplace_shared<gtsam::PriorFactor<gtsam::Point3>>(
              key, point, noise);
          break;
        }
        default: {
    ...
    if (symb.chr() == 'l') {
      ...
      for (const auto& prefix : key_prefixes_to_prior) {
        add_prior(gtsam::Symbol(prefix, symb.index()));
        add_prior(gtsam::Symbol(prefix, first_key.index()));
      }
    }

    if (nfg.empty()) {
      LOG(ERROR) << "Smoother recovery failed: no valid recovery priors could "
                    "be created for variable "
                 << symb.chr() << symb.index();
      return false;
    }
...
void VioBackend::postDebug(
  ...
  if (log_output_) {
    try {
      ...
      LOG(WARNING) << "Skipping sparsity statistics due to cheirality: "
                   << e.what();
    } catch (const gtsam::StereoCheiralityException& e) {
      LOG(WARNING) << "Skipping sparsity statistics due to stereo cheirality: "
                   << e.what();
    }
    ...
    try {
      ...
      LOG(WARNING) << "Skipping smart factor statistics due to cheirality: "
                   << e.what();
    } catch (const gtsam::StereoCheiralityException& e) {
      LOG(WARNING) << "Skipping smart factor statistics due to stereo cheirality: "
                   << e.what();
    }
  }
  ...
  if (VLOG_IS_ON(10)) {
    ...
    try {
      ...
      LOG(WARNING) << "Skipping optimization error logging due to cheirality: "
                   << e.what();
    } catch (const gtsam::StereoCheiralityException& e) {
      LOG(WARNING)
          << "Skipping optimization error logging due to stereo cheirality: "
          << e.what();
    }
  }
}
  ...
  for (auto it = factor_graph_output->begin();
    ...
    if (*it) {
      if ((*it)->find(key) != (*it)->end()) {
        ...
        // Landmark priors can be present after previous smoother recovery and
        // must be removed with the cheirality landmark.
