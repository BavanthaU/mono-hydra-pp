...
bool RegularVioBackend::addVisualInertialStateAndOptimize(
  ...
  // Add sparse depth factors before optimization so they constrain the current
  // keyframe update instead of lagging by one backend iteration.
  if (regular_vio_params_.use_sparse_depth_factors_ &&
      !sparse_depth_meas.empty()) {
  ...
  if (is_smoother_ok) {
    ...
    // factors and must be deleted from the factor graph.
...
void RegularVioBackend::addSparseDepthFactors(
  ...
  const auto max_factors = regular_vio_params_.sparse_depth_max_factors_per_kf_;
  size_t count = 0;
  ...
  // Mono calibration from stereo calib.
  gtsam::Cal3_S2 mono_cal(stereo_cal_->fx(),
                          stereo_cal_->fy(),
                          stereo_cal_->skew(),
                          stereo_cal_->px(),
                          stereo_cal_->py());
  ...
  for (const auto& m : sparse_depth_meas) {
    ...
    const double sigma = regular_vio_params_.sparse_depth_sigma_a_ +
                         regular_vio_params_.sparse_depth_sigma_b_ * m.depth_meas_;
    if (sigma <= 0.0) {
    ...
    auto noise = gtsam::noiseModel::Robust::Create(
        gtsam::noiseModel::mEstimator::Huber::Create(
            regular_vio_params_.sparse_depth_huber_k_),
        base);
    ...
    if (!state_.exists(lmk_sym) && !new_values_.exists(lmk_sym)) {
      ...
      gtsam::Point3 p_cam(x, y, z);
      gtsam::Point3 p_body = B_Pose_leftCamRect_.transformFrom(p_cam);
      gtsam::Point3 p_world = W_Pose_B_k.transformFrom(p_body);
      new_values_.insert<gtsam::Point3>(lmk_sym, p_world);
      initialized_here = true;
      ++initialized_landmarks;

      // A depth residual alone is 1D and leaves a newly created Point3
      // underconstrained. Add the matching pixel-bearing factor so RGB-D
      // initialized landmarks are locally well-posed in the fixed-lag graph.
    ...
    // Reprojection gate on existing points.
    if (!initialized_here) {
      ...
      gtsam::Pose3 W_Pose_C = W_Pose_B_k * B_Pose_leftCamRect_;
      gtsam::Point3 p_cam = W_Pose_C.transformTo(p_world);
      if (p_cam.z() <= 1e-6) {
        ++skipped_behind_camera;
        continue;
      }
      const double u_proj = mono_cal.fx() * p_cam.x() / p_cam.z() + mono_cal.px();
      const double v_proj = mono_cal.fy() * p_cam.y() / p_cam.z() + mono_cal.py();
      const double err_px =
          std::hypot(u_proj - m.u_, v_proj - m.v_);
      if (err_px > regular_vio_params_.sparse_depth_reproj_thresh_px_) {
