...
bool BackendParams::parseYAMLVioBackendParams(const YamlParser& yaml_parser) {
  ...
  if (yaml_parser.hasParam("sparse_depth_max_factors_per_kf")) {
    ...
    if (max_factors > 0) {
      sparse_depth_max_factors_per_kf_ = static_cast<size_t>(max_factors);
    }
  }
  ...
  if (yaml_parser.hasParam("sparse_depth_reproj_thresh_px")) {
    yaml_parser.getYamlParam("sparse_depth_reproj_thresh_px",
                             &sparse_depth_reproj_thresh_px_);
  }
...
bool BackendParams::equalsVioBackendParams(const BackendParams& vp2,
  ...
  return
      ...
      (use_sparse_depth_factors_ == vp2.use_sparse_depth_factors_) &&
      (sparse_depth_max_factors_per_kf_ ==
       vp2.sparse_depth_max_factors_per_kf_) &&
      (fabs(sparse_depth_sigma_a_ - vp2.sparse_depth_sigma_a_) <= tol) &&
      (fabs(sparse_depth_sigma_b_ - vp2.sparse_depth_sigma_b_) <= tol) &&
      (fabs(sparse_depth_huber_k_ - vp2.sparse_depth_huber_k_) <= tol) &&
      (fabs(sparse_depth_reproj_thresh_px_ -
            vp2.sparse_depth_reproj_thresh_px_) <= tol);
}
...
void BackendParams::printVioBackendParams() const {
  ...
  PipelineParams::print(
      ...
      mono_translation_scale_factor_,
      "Use Sparse Depth Factors",
      use_sparse_depth_factors_,
      "Sparse Depth Max Factors Per KF",
      sparse_depth_max_factors_per_kf_,
      "Sparse Depth Sigma A",
      sparse_depth_sigma_a_,
      "Sparse Depth Sigma B",
      sparse_depth_sigma_b_,
      "Sparse Depth Huber K",
      sparse_depth_huber_k_,
      "Sparse Depth Reproj Threshold Px",
      sparse_depth_reproj_thresh_px_);
  LOG(INFO) << out.str();
