...
FrameId LoopClosureDetector::processAndAddMonoFrame(
  ...
  switch (lcd_params_.pose_recovery_type_) {
    case PoseRecoveryType::k5ptRotOnly: {
      ...
      getNewFeaturesAndDescriptors(frame.img_,
                                   frame.detection_mask_,
                                   &keypoints,
                                   &descriptors_mat);
    ...
    case PoseRecoveryType::kPnP: {
      ...
      const cv::Mat detection_mask =
          getValidDetectionMask(frame.img_, frame.detection_mask_);
...
FrameId LoopClosureDetector::processAndAddStereoFrame(
  ...
  OrbDescriptor descriptors_mat;
  OrbDescriptorVec descriptors_vec;
  getNewFeaturesAndDescriptors(stereo_frame.left_frame_.img_,
                               stereo_frame.left_frame_.detection_mask_,
                               &keypoints,
                               &descriptors_mat);
  descriptorMatToVec(descriptors_mat, &descriptors_vec);
...
FrameId LoopClosureDetector::processAndAddRgbdFrame(
  ...
  OrbDescriptor descriptors_mat;
  OrbDescriptorVec descriptors_vec;
  getNewFeaturesAndDescriptors(rgbd_frame.intensity_img_.img_,
                               rgbd_frame.intensity_img_.detection_mask_,
                               &keypoints,
                               &descriptors_mat);
  descriptorMatToVec(descriptors_mat, &descriptors_vec);
...
void LoopClosureDetector::getNewFeaturesAndDescriptors(
  ...
  const cv::Mat valid_detection_mask =
      getValidDetectionMask(img, detection_mask);
  orb_feature_detector_->detectAndCompute(
      img, valid_detection_mask, *keypoints, *descriptors_mat);
}

/* ------------------------------------------------------------------------ */
cv::Mat LoopClosureDetector::getValidDetectionMask(
    const cv::Mat& img,
  ...
  if (detection_mask.size() != img.size()) {
    LOG_FIRST_N(WARNING, 5)
        << "LoopClosureDetector: detection mask size does not match image; "
        << "ignoring LCD mask. image=" << img.cols << "x" << img.rows
        << " mask=" << detection_mask.cols << "x" << detection_mask.rows;
    return cv::Mat();
  }

  if (detection_mask.type() != CV_8UC1) {
    LOG_FIRST_N(WARNING, 5)
        << "LoopClosureDetector: detection mask must be CV_8UC1; ignoring "
        << "LCD mask with type " << detection_mask.type();
    return cv::Mat();
  }
...
void LoopClosureDetector::addOdometryFactorAndOptimize(
  ...
  try {
    ...
    LOG(ERROR) << "CheiralityException in LCD addOdometryFactorAndOptimize: "
               << e.what();
  } catch (const gtsam::StereoCheiralityException& e) {
    LOG(ERROR) << "StereoCheiralityException in LCD addOdometryFactorAndOptimize: "
               << e.what();
  } catch (const std::exception& e) {
    LOG(ERROR) << "Exception in LCD addOdometryFactorAndOptimize: " << e.what();
  }
...
void LoopClosureDetector::addLoopClosureFactorAndOptimize(
  ...
  try {
    ...
    LOG(ERROR) << "CheiralityException in LCD addLoopClosureFactorAndOptimize: "
               << e.what();
  } catch (const gtsam::StereoCheiralityException& e) {
    LOG(ERROR) << "StereoCheiralityException in LCD addLoopClosureFactorAndOptimize: "
               << e.what();
  } catch (const std::exception& e) {
    LOG(ERROR) << "Exception in LCD addLoopClosureFactorAndOptimize: " << e.what();
  }
}
