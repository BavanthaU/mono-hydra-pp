...
RgbdImuPipeline::RgbdImuPipeline(const VioParams& params,
  ...
  if (params.frontend_params_.rgbd_use_depth_only_frontend_) {
    bool sparse_depth_enabled = false;
    size_t backend_sparse_max = 0u;
    if (params.backend_params_) {
      sparse_depth_enabled = params.backend_params_->use_sparse_depth_factors_;
      backend_sparse_max =
          params.backend_params_->sparse_depth_max_factors_per_kf_;
    }
    if (!sparse_depth_enabled) {
      LOG(WARNING)
          << "RGBD depth-only frontend enabled but sparse depth factors are "
             "disabled; backend will rely on mono factors (depth mainly used "
             "for PnP). Consider enabling use_sparse_depth_factors in "
             "BackendParams.yaml.";
    }
    if (params.frontend_params_.rgbd_use_dense_depth_factors_ &&
        sparse_depth_enabled &&
        backend_sparse_max < params.frontend_params_.rgbd_dense_depth_max_meas_) {
      LOG(WARNING)
          << "RGBD dense depth factors requested, but backend cap "
          << backend_sparse_max
          << " < rgbd_dense_depth_max_meas "
          << params.frontend_params_.rgbd_dense_depth_max_meas_
          << "; increase sparse_depth_max_factors_per_kf to use more depth "
             "constraints.";
    }
  }
