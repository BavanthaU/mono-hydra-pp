...
/* ************************************************************************* */
TEST(testFrame, copyPreservesDetectionMask) {
  Frame f(0, 0, CameraParams(), Mat(4, 4, CV_8UC1, Scalar(0)));
  f.detection_mask_ = Mat(4, 4, CV_8UC1, Scalar(255));

  Frame copy(f);
  ASSERT_FALSE(copy.detection_mask_.empty());
  EXPECT_EQ(copy.detection_mask_.at<uint8_t>(0, 0), 255);

  f.detection_mask_.setTo(0);
  EXPECT_EQ(copy.detection_mask_.at<uint8_t>(0, 0), 255);
}
