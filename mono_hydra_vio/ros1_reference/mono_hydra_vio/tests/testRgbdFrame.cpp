...
TEST_F(TestRgbdFrame, PreservesDetectionMask) {
  const cv::Mat mono_img(4, 4, CV_8UC1, cv::Scalar(10));
  const cv::Mat depth_img(4, 4, CV_32FC1, cv::Scalar(2.0f));

  CameraParams params;
  DepthFrame dframe(5, 10, depth_img);
  Frame lframe(5, 10, params, mono_img);
  lframe.detection_mask_ = cv::Mat(4, 4, CV_8UC1, cv::Scalar(255));
  lframe.detection_mask_.at<uint8_t>(1, 1) = 0;

  RgbdFrame frame(5, 10, lframe, dframe);
  ASSERT_FALSE(frame.intensity_img_.detection_mask_.empty());
  EXPECT_EQ(frame.intensity_img_.detection_mask_.at<uint8_t>(1, 1), 0);

  const auto stereo_frame = frame.getStereoFrame();
  ASSERT_TRUE(stereo_frame != nullptr);
  ASSERT_FALSE(stereo_frame->left_frame_.detection_mask_.empty());
  EXPECT_EQ(stereo_frame->left_frame_.detection_mask_.at<uint8_t>(1, 1), 0);
}
