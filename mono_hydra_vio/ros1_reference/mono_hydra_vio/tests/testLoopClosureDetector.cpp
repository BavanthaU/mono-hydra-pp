...
TEST_F(LCDFixture, processAndAddStereoFrameUsesDetectionMask) {
  CHECK(lcd_detector_);
  const auto& cache = lcd_detector_->getFrameCache();
  EXPECT_EQ(cache.size(), 0);

  match1_stereo_frame_->left_frame_.detection_mask_ =
      cv::Mat(match1_stereo_frame_->left_frame_.img_.size(),
              CV_8UC1,
              cv::Scalar(0));
  ...
  auto stereo_lcd_frame =
      std::dynamic_pointer_cast<StereoLCDFrame>(cache.getFrame(0));
  ASSERT_TRUE(stereo_lcd_frame != nullptr);
  EXPECT_EQ(stereo_lcd_frame->keypoints_.size(), 0u);
  EXPECT_EQ(stereo_lcd_frame->descriptors_vec_.size(), 0u);
  EXPECT_EQ(stereo_lcd_frame->descriptors_mat_.rows, 0);
}
