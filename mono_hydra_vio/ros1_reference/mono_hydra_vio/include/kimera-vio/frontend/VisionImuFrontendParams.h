...
struct FrontendParams : public PipelineParams {
 ...
 public:
  ...
  //! If true, RGBD frontend skips pseudo-stereo measurements (depth-only mode).
  bool rgbd_use_depth_only_frontend_ = false;
  ...
  //! Max number of sparse depth measurements per keyframe (RGBD).
  size_t rgbd_sparse_depth_max_meas_ = 30u;
  //! Enable denser depth measurements for backend factors (RGBD).
  bool rgbd_use_dense_depth_factors_ = false;
  //! Max number of dense depth measurements per keyframe (RGBD).
  size_t rgbd_dense_depth_max_meas_ = 120u;
};
