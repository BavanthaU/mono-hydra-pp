...
class RgbdVisionImuFrontend : public VisionImuFrontend {
 ...
 private:
  ...
  void applySemanticMask(cv::Mat* detection_mask) const;
 ...
 private:
  ...
  const bool use_depth_only_frontend_;
  const size_t sparse_depth_max_meas_;
  const bool use_dense_depth_factors_;
  const size_t dense_depth_max_meas_;

  cv::Mat semantic_mask_;
  bool semantic_mask_loaded_ = false;
};
