    ...
    : mPrivateNode("~")
{
    ...
    mPrivateNode.param("publish_tf", mbPublishTf, true);
    ...
    const cv::FileNode staticInitTimeNode = fsSettings["INI.nStaticInitTime"];
    mnStaticInitTime = staticInitTimeNode.empty()
                         ? 0.0f
                         : static_cast<float>(staticInitTimeNode);
...
bool System::initialize(const ImageData& Image, const std::vector<ImuData>& vImuData)
{
    ...
    if (ang.norm()*180./M_PI<mnAngleThrd && len.norm()<mnLengthThrd)
    {
        ...
        if (mnStaticInitTime <= 0.0f || Dt < mnStaticInitTime)
            return false;

        ROS_INFO("Static initialization after %.3f seconds of stationary IMU data", Dt);
    }
...
void System::run()
{
    ...
    const ros::Time measurement_stamp(pMeasurements.first.Timestamp);
    ...
    ROS_INFO_THROTTLE(
        5.0,
        "RVIO2 t=%.3f p_Gk=(%.3f %.3f %.3f) q_kG=(%.4f %.4f %.4f %.4f) td=%.6f",
        measurement_stamp.toSec(),
        pGk(0),
        pGk(1),
        pGk(2),
        qkG(0),
    ...
    if (mbPublishTf)
    {
        // RVIO2's q_kG/p_Gk state is a body pose. Keep the legacy frame ids for
        // standalone RVIO2 visualization, but allow launch files to disable this
        // TF when another estimator owns the odom tree.
        geometry_msgs::TransformStamped transformStamped;
        transformStamped.header.stamp = measurement_stamp;
    ...
    pose.header.stamp = measurement_stamp;
    ...
    path.header.stamp = measurement_stamp;
